// Zero-dependency static file server for the Playwright e2e fixtures. Serves the
// package root over HTTP so the fixture can import the built ESM bundle
// (`/dist/index.js`, `/dist/dom/index.js`) with real module resolution — `file://`
// blocks module CORS, so a real origin is required. Started by `webServer` in
// `playwright.config.ts`; not part of the shipped package.
...
const server = createServer(async (req, res) => {
  ...
  const relative = requestPath === '/' ? '/playground/index.html' : requestPath
  ...
  if (err.code === 'EADDRINUSE') {
    process.stderr.write(
      `static-server: port ${port} is already in use.\n` +
        ...
        `  • Stop it: kill $(lsof -t -i:${port})   (or pick another port: PORT=4174 pnpm dev:play)\n`,
    )
    process.exit(1)
  }
  throw err
